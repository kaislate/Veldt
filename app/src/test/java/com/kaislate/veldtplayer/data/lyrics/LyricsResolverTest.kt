...
class LyricsResolverTest {
    ...
    @Test fun `a local track with a sidecar hit asks only sidecar`() = runTest {
        ...
        val resolver = resolver(calls, sidecarResult = Lyrics.Plain("sidecar text"))
        val result = resolver.resolve(song(localSourceId, "1"))
        assertEquals(ResolvedLyrics(Lyrics.Plain("sidecar text"), LyricsSource.SIDECAR), result)
    ...
    @Test fun `a local track with no sidecar but an embedded hit asks sidecar then embedded`() = runTest {
        val calls = mutableListOf<String>()
        val resolver = resolver(calls, embeddedResult = Lyrics.Plain("embedded text"))
        val result = resolver.resolve(song(localSourceId, "1"))
        assertEquals(ResolvedLyrics(Lyrics.Plain("embedded text"), LyricsSource.EMBEDDED), result)
    ...
    @Test fun `a local track that falls through to lrclib asks sidecar, embedded, then lrclib`() = runTest {
        val calls = mutableListOf<String>()
        val resolver = resolver(calls, lrclibResult = Lyrics.Plain("lrclib text"))
        val result = resolver.resolve(song(localSourceId, "1"))
        assertEquals(ResolvedLyrics(Lyrics.Plain("lrclib text"), LyricsSource.LRCLIB), result)
    ...
    @Test fun `a local track never asks server`() = runTest {
        val calls = mutableListOf<String>()
        val resolver = resolver(calls, serverResult = Lyrics.Plain("should never be reached"))
    ...
    @Test fun `a remote track with a server hit asks only server`() = runTest {
        val calls = mutableListOf<String>()
        val resolver = resolver(calls, serverResult = Lyrics.Plain("server text"))
        val result = resolver.resolve(song("navidrome-1", "s1"))
        assertEquals(ResolvedLyrics(Lyrics.Plain("server text"), LyricsSource.SERVER), result)
    ...
    @Test fun `a remote track never asks sidecar or embedded`() = runTest {
        ...
        val resolver = resolver(
            calls,
            sidecarResult = Lyrics.Plain("should never be reached"),
            embeddedResult = Lyrics.Plain("should never be reached"),
        )
